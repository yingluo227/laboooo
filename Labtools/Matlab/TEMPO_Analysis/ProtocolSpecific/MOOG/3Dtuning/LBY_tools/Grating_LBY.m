...
for spa = 1:length(unique_spatFre)
        PSTH.bin_rate_aov{spa} = nan*ones(length(unique_orient),nBins,max_reps);
        pc = 0;
        for i = 1:length(unique_orient)
            % initialize
            PSTH.rate{spa,i} = [];
            PSTH.rate_all{spa,i} = [];
            spk_data{spa,i} = []; % raw spike count (transform spike_spk_data to a more clear way)
            PSTH.bin_rate{spa,i} = []; % spike counts per time window (for PSTH across trials)
            
            % find specified conditions with repetitions -> pack the data
            select = (temp_stimType == 1)&(temp_spatFre == unique_spatFre(spa))&(temp_orient == unique_orient(i)) ;
            if sum(select)>0
                spk_data{spa,i} = spike_data(:,select);
                pc = pc+1;
                % for contour
                rate_aov{spa,pc} = [];
                rate_aov{spa,pc} =  sum(spk_data{spa,i}(stimOnT(1)+tBeg:stimOffT(1)-tEnd,:),1)/((unique_duration(1,1)-tBeg-tEnd)/1000);
                resp_sse(spa,pc) = sum((rate_aov{spa,pc} - mean(rate_aov{spa,pc})).^2); % for DDI
                resp_trialnum(spa,pc)= size(rate_aov{spa,pc},2); % for DDI
            else
                disp('There must be something wrong, no trials for this condition!');
            end
            ...
            PSTH.bin_rate{spa,i} = PSTH_smooth(nBins, PSTH_onT, timeWin, timeStep, spk_data{spa,i}, 2, gau_sig);
            temp = PSTH_smooth( nBins, PSTH_onT, timeWin, timeStep, spk_data{spa,i}, 2, gau_sig);
            try
                PSTH.bin_rate_aov{spa}(pc,:,1:size(temp,2)) = temp;
            catch
                keyboard;
            end
            temp = PSTH_smooth( nBinsPCA, stimOnT, PCAWin, PCAStep, mean(spk_data{spa,i},2), 1, 0);
            PSTH.bin_rate_PCA{spa}(pc,:,1:size(temp,2)) = temp;
            
            PSTH.bin_mean_rate_PCA{spa}(pc,:) = nanmean(PSTH.bin_rate_PCA{spa}(pc,:,:),3);
            PSTH.bin_mean_rate_aov{spa}(pc,:) = nanmean(PSTH.bin_rate_aov{spa}(pc,:,:),3);
            
            % calculate correlation coefficient between response and V/A curve
            [R,P] = corrcoef(v_timeProfile,PSTH.bin_mean_rate_aov{spa}(pc,:));
            PSTH.coef_v{spa}(1,pc) = R(1,2);
            PSTH.coef_v{spa}(2,pc) = P(1,2);
            [R,P] = corrcoef(a_timeProfile,PSTH.bin_mean_rate_aov{spa}(pc,:));
            PSTH.coef_a{spa}(1,pc) = R(1,2);
            PSTH.coef_a{spa}(2,pc) = P(1,2);
            
            a = mean(PSTH.bin_rate{spa,i},2);
            PSTH.bin_mean_rate(spa,i,:) = mean(PSTH.bin_rate{spa,i},2);
            PSTH.spk_spk_data_bin_vector = PSTH.bin_mean_rate;
            PSTH.spk_spk_data_bin_vector([1,5],2:end,:) = 0;
            bin_mean_rate_std(spa,i,:) = std(PSTH.bin_rate{spa,i},0,2);
            PSTH.bin_mean_rate_ste(spa,i,:) = bin_mean_rate_std(spa,i,:)/sqrt(size(PSTH.bin_rate{spa,i},2));
            
            % pack spk_data for contour
            PSTH.rate{spa,i} = sum(spk_data{spa,i}(stimOnT(1)+tBeg:stimOffT(1)-tEnd,:),1)/((unique_duration(1,1)-tBeg-tEnd)/1000); % rates
            PSTH.mean_rate(spa,i) = mean(PSTH.rate{spa,i}(:));% for countour plot and ANOVA
            PSTH.rate_all{spa,i} = sum(spk_data{spa,i}(stimOnT(1):stimOffT(1),:),1)/(unique_duration(1,1)/1000); % rates of all duration
            PSTH.mean_rate_all(spa,i) = mean(PSTH.rate_all{spa,i}(:));% for models
            vector_rate{spa,i} = sum(spk_data{spa,i}(stimOnT(1)+tBeg:stimOnT(1)+tBeg+unique_duration(1,1)/2,:),1)/((unique_duration(1,1)/2)/1000); % rates
            
        end
        % preliminary analysis
        PSTH.time_profile{spa} = squeeze(sum(sum(PSTH.bin_mean_rate(spa,i,:),1),2));
        PSTH.bin_mean_rate_aov_cell{spa} = mean(PSTH.bin_mean_rate_aov{spa},1);
        maxSpkRealMean(spa) = max(PSTH.mean_rate(spa,:));
        minSpkRealMean(spa) = min(PSTH.mean_rate(spa,:));
        PSTH.maxSpkRealbin_mean(spa) = max(max(PSTH.bin_mean_rate(spa,:,:)));
        PSTH.minSpkRealbin_mean(spa) = min(min(PSTH.bin_mean_rate(spa,:,:)));
        PSTH.maxSpkRealBinAll(spa) = max(PSTH.bin_rate_aov{spa}(:));
        PSTH.maxSpkRealbin_meanSte(spa) = max(max(PSTH.bin_mean_rate_ste(spa,:,:)));
        
        % calculate DDT & preferred directions for tuning
    numReps(spa) = max(cell2mat(cellfun(@length,rate_aov(spa,:),'UniformOutput',false)));
    for nn = 1:length(rate_aov(spa,:))
        if length(rate_aov{spa,nn})<numReps(spa)
            rate_aov{spa,nn} = [rate_aov{spa,nn} nan*ones(1,(numReps(spa)-length(rate_aov{spa,nn})))];
        end
    end
    spk_data_count_rate_anova_trans{spa} = reshape(cell2mat(rate_aov(spa,:)),[numReps(spa),length(rate_aov(spa,:))]);
    p_anova_dire(spa) = anova1(spk_data_count_rate_anova_trans{spa},'','off'); % tuning anova
    resp_std(spa) = sum(resp_sse(spa,:))/(sum(resp_trialnum(spa,:))-length(unique_orient));
    DDI(spa) = (maxSpkRealMean(spa)-minSpkRealMean(spa))/(maxSpkRealMean(spa)-minSpkRealMean(spa)+2*sqrt(resp_std(spa)));
    [spatial,orientation] = vectorsum(PSTH.vector_rate{spa});
    preferDire{spa} = [Azi, Ele, Amp];
    ...
    pre_diff{spa} = angleDiff(azis,eles,ones(1,40),ones(1,40)*preferDire{spa}(1),ones(1,40)*preferDire{spa}(2),ones(1,40));
...
for spa = 1:length(unique_spatFre)
    for ttt = 1:length(unique_tempFre)
        %     spk_data_bin_rate_mean_minusSpon{spa} = PSTH.spk_data_bin_mean_rate_aov{spa}-repmat(PSTH.spon_spk_data_bin_mean_rate',size(PSTH.spk_data_bin_mean_rate_aov{spa},1),1);
...
result = PackResult(FILE, PATH, SpikeChan, unique_tempFre, unique_spatFre, unique_stimType,Protocol, ... % Obligatory!!
    ...
    markers,...
    timeWin, timeStep, tOffset1, tOffset2,nBins,Bin,PCAStep,PCAWin,nBinsPCA, ... % PSTH slide window info
    meanSpon, PSTH, p_anova_dire, DDI,preferDire) % PSTH and mean FR info
%     p_anova_dire, DDI,preferDire); 
