...
for k = 1:length(unique_stimType)
    if ~isempty(PSTH.peak{k})
        ...
        if Protocol == DIRECTION_TUNING_3D
            ss = [str, '_T'];
            saveas(gcf,['Z:\Data_analyses\Recording_data\',PSTH.monkey,'\3D_Tuning\Translation\' ss], 'emf');
        elseif Protocol == ROTATION_TUNING_3D
            ss = [str, '_R'];
            saveas(gcf,['Z:\Data_analyses\Recording_data\',PSTH.monkey,'\3D_Tuning\Rotation\' ss], 'emf');
        end
    end
end
%}
